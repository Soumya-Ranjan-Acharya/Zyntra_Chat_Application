...
// @desc    Register or update device public identity key and fingerprint
// @route   POST /api/auth/keys
// @access  Private
export const uploadIdentityKey = async (req, res, next) => {
  try {
    const { publicKey, keyFingerprint } = req.body;

    if (!publicKey || !keyFingerprint) {
      return res.status(400).json({
        success: false,
        message: 'publicKey and keyFingerprint are required',
      });
    }
...
// @desc    Get public key and fingerprint for a user by id or username
// @route   GET /api/auth/keys/:identifier
...
export const getPublicKey = async (req, res, next) => {
  try {
    ...
    let user = await User.findOne({ primaryUsername: clean });
